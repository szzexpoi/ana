...
class Evaluation:
    ...
    def run(self, show=False):
        ...
        for chk_id in self.chk_numbers:
            attention_distribution = torch.zeros(9,7,7) # saving the predicted attention for different actions
            ...
            for scene_scope, items in self.config['task_list'].items():
                ...
                for task_scope in items:
                    ...
                    for i_episode in range(self.config['num_episode']):
                        ...
                        while not terminal:
                            if self.method != "random":
                                policy, value, state, att = self.method_class.forward_policy(
                            ...
                            attention_distribution[action] = attention_distribution[action]+att.data.cpu()
                    ...
                    if show:
                        ...
                        self.save_video_att(ep_lengths, ep_actions, ep_att, ep_start, ind_succeed_ep, chk_id, env, scene_scope, task_scope)
                    ...
                    if show:
                        ...
                        self.save_video_att(ep_lengths, ep_actions, ep_att, ep_start, ep_failed_selec, chk_id, env, scene_scope, task_scope, success=False)
            ...
            attention_distribution = attention_distribution.view(9,49)
            attention_distribution = attention_distribution/attention_distribution.max(-1,keepdim=True)[0].expand_as(attention_distribution)
            attention_distribution = attention_distribution.view(9,7,7).numpy()
            np.save(self.config['base_path'] + 'attention' + str(chk_id),attention_distribution)
            # break
