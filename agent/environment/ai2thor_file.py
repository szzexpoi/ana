...
class THORDiscreteEnvironment(Environment):
    ...
    def _get_state(self, state_id):
        # # read from hdf5 cache (original)        
        return self.h5_file['resnet_feature'][state_id].reshape(-1)[:, np.newaxis]
    ...
    def render_target(self, mode):
        if self.method == 'word2vec_notarget_lstm' or self.method == 'aop' or self.method == 'aop_we' or self.method == 'word2vec' or self.method == 'word2vec_nosimi' or self.method == 'word2vec_noconv' or self.method == "gcn":
