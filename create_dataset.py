...
# transformation for spatial feature
...
pooler = torch.nn.AdaptiveMaxPool2d((7,7)).to("cuda")
            ...
            for i in range(3):
                ...
                for a in range(rotation_possible_inplace):
                    ...
                    if ('resnet_feature' not in h5_file.keys() or args['force']) and not args['view']:
                        spatial_inp = Image.fromarray(state.frame)
                        spatial_inp = spatial_transform(spatial_inp)
                        spatial_inp = spatial_inp.cuda()
                        feature = resnet_trained(spatial_inp.unsqueeze(0))
                        feature = pooler(feature)
...
def extract_object_feature(resnet_trained, h, w):
    ...
    # 512 is the resnet18 feature size
    object_feature = np.zeros((len(OBJECT_IDS), 512,7,7), dtype=np.float32) # for spatial features
    ...
    for filepath in glob.glob('data/objects/*.jpg'):
        ...
        spatial_inp = Image.open(filepath).convert('RGB')
        spatial_inp = spatial_inp.resize((w, h))
        spatial_inp = spatial_transform(spatial_inp)
        spatial_inp = spatial_inp.cuda()
        feature = resnet_trained(spatial_inp.unsqueeze(0))
        feature = pooler(feature)
...
def main():
    ...
    # use spatial features from resnet18/resnet50
    resnet_spatial = models.resnet18(pretrained=True)
    resnet_spatial = torch.nn.Sequential(*list(resnet_spatial.children())[:-2])
    resnet_spatial.eval()
    resnet_spatial = resnet_spatial.to("cuda")
    ...
    for idx, name in enumerate(pbar_names):
        ...
        if custom_scene:
            states = create_states(h5_file, resnet_spatial, resnet_places,
